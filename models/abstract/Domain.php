 ...
 */
abstract class PSP_Models_Abstract_Domain {
	...
	public function __construct( $properties = null ) {
		...
		if ( ! empty( $properties ) ) {
			foreach ( (array) $properties as $key => $value ) // assign value to key
			{
				$this->$key = $value;
			}
		}
	...
	public function __call( $method, array $arguments ) {
		...
		if ( preg_match( '/^get(\w+?)?$/', $method, $matches ) ) {
			...
			// if there is no property, throw exception
